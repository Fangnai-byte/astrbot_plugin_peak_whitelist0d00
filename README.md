...
- 白名单成员、群管理员自动放行
- **群黑名单**：整个群的消息一律拦截（最高优先级）
- **成员黑名单**：指定成员一律拦截，无论是否白名单/管理员/时段
- 目标群、白名单、黑名单、时段全部**可配置**（WebUI 插件配置面板，修改即时生效）
- 可设置仅工作日生效（周末不拦截）
...
| `group_blacklist` | 群黑名单：整个群的消息一律拦截 | 空 |
| `whitelist` | 白名单成员 QQ 列表 | 空 |
| `blacklist` | 黑名单成员 QQ 列表（最高优先级拦截） | 空 |
...
- 黑名单拦截优先级最高：被拉黑的人/群无论何时何地都不会触发 bot

## License

MIT License

> 最后更新：2026-09-02（v1.3.1：新增群黑名单与成员黑名单）
